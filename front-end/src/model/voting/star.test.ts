import { VoteAndBallots, VoteOption, tallyIndividual, tallyOrder } from "../vote";
...
test('STAR elects most popular candidate', () => {
    ...
    expect(tallyIndividual(vote)).toEqual(['option-3']);
});
...
test('STAR gets runoff right', () => {
    ...
    expect(tallyIndividual(vote)).toEqual(['option-1']);
...
test('STAR generates sensible replacements', () => {
    ...
    let [winner] = tallyIndividual(vote);
    expect(winner).toEqual('option-3');
    let [replacement] = tallyIndividual({ ...vote, vote: { ...vote.vote, resigned: [winner] } });
...
test('STAR generates consistent replacements', () => {
    ...
    for (let expectedWinner of order) {
        let [winner] = tallyIndividual({ ...vote, vote: { ...vote.vote, resigned } });
