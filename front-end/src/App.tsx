...
class VoteRoute extends FetchedStateComponent<{ match: any, history: any, permissions: Permission[] }, VoteRouteState> {
  ...
  async fetchState(): Promise<VoteRouteState> {
    ...
    return { vote: data, ballotCast: false, suspiciousBallots, permissions: this.permissions };
  }
    ...
    if (data.createdVote) {
      ...
      return <MakeVotePage
        ...
        allowedTallyingAlgorithms={
          getBallotKind(data.draftVote.type) === "choose-one" ? ["first-past-the-post", "sainte-lague", "simdem-sainte-lague"] :
          getBallotKind(data.draftVote.type) === "ranked-choice" ? ["stv", "stv-g"] : 
          getBallotKind(data.draftVote.type) === "rate-options" ? ["star", "spsv"] : []}
